    ...
    class InvoicesController < BaseController
      def index
        invoices = if current_user.superadmin?
          ...
          Invoice.includes(:user, invoice_file_attachment: :blob).order(invoice_date: :asc, id: :asc)
        elsif current_user.role == 'admin'
          # adminはスーパー管理者とokiユーザー以外のインボイスを閲覧可能
          Invoice.includes(:user, invoice_file_attachment: :blob)
                    .joins(:user)
          ...
          current_user.invoices.includes(invoice_file_attachment: :blob).order(invoice_date: :asc, id: :asc)
        end
